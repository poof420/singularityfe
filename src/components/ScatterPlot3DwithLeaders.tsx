...
const mockData: Point[] = Array.from({ length: 500 }, () => ({
  x: Math.random() * 10 - 5,
  y: Math.random() * 10 - 5,
  z: Math.random() * 10 - 5,
  value: Math.random(),
  ...
  return (
    <group>
      {data.map((point: Point, index: number) => {
        const isVisible =
          (sliceAxis === 'x' && point.x > slicePosition) ||
...
function SlicePlane({
  ...
  return (
    <Plane args={[10, 10]} {...planeConfig[axis]}>
      <meshBasicMaterial
        color="#00FFFF"
        opacity={0.2}
        transparent
      ...
      <div className="fixed bottom-4 left-4 flex flex-col space-y-2">
        <div className="flex space-x-2">
          {(['x', 'y', 'z'] as const).map((axis) => (
            <Button
              key={axis}
              className={`bg-[#00FFFF] text-black hover:bg-[#00CCCC] ${
                sliceAxis === axis ? 'ring-2 ring-white' : ''
              }`}
